...
mean = [0.8963, 0.9022, 0.9130]
std = [0.1191, 0.0912, 0.4594]
...
class urine_labeled_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.target_transform is not None:
            label_tensor = self.target_transform(self.imagenet[index][1])
        return image_tensor, label_tensor
...
def get_urine_data(positive_label_list):
    target_transform = lambda x: 1 if x in positive_label_list else 0
    # train_labeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/git/vpu-tilt/data/train', data_class ='p',target_transform=target_transform)
    # train_unlabeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/git/vpu-tilt/data/train', data_class ='u', target_transform=target_transform)
    # val_unlabeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/git/vpu-tilt/data/test', data_class ='u', target_transform=target_transform)
    # val_labeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/git/vpu-tilt/data/test', data_class ='p',target_transform=target_transform)
    # test_dataset = urine_labeled_dataset('/bigdata/projects/beidi/dataset/urine/Urine_putest',target_transform=target_transform)

    train_labeled_dataset = urine_labeled_dataset('/fast/beidi/data/tile128_rand100/train', data_class='p',target_transform=target_transform)
    train_unlabeled_dataset = urine_labeled_dataset('/fast/beidi/data/tile128_rand100/train', data_class='u',
                                                    target_transform=target_transform)
    val_unlabeled_dataset = urine_labeled_dataset('/fast/beidi/data/tile128_rand100/test', data_class='u',
                                                  target_transform=target_transform)
    val_labeled_dataset = urine_labeled_dataset('/fast/beidi/data/tile128_rand100/test', data_class='p',
                                                target_transform=target_transform)
    test_dataset = urine_labeled_dataset('/fast/beidi/vpu-tilt/Urine_putest',  target_transform=target_transform)
    return train_labeled_dataset, train_unlabeled_dataset, val_labeled_dataset, val_unlabeled_dataset, test_dataset


def get_urine_loaders(positive_label_list, batch_size=500):
    train_labeled_dataset, train_unlabeled_dataset, val_labeled_dataset, val_unlabeled_dataset, test_dataset = get_urine_data(positive_label_list=positive_label_list)
